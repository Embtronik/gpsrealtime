...
// â”€â”€â”€ Fetch de una pÃ¡gina â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function fetchPage(page) {
  _currentPage = page;
  const tbody = document.querySelector('#tableClientes tbody');
  tbody.innerHTML = '<tr><td colspan="43" class="text-center py-3"><div class="spinner-border spinner-border-sm"></div> Buscando...</td></tr>';
  ...
  try {
    ...
    document.querySelector('#tableClientes tbody').innerHTML =
      `<tr><td colspan="43" class="text-center text-danger">${escHtml(e.message)}</td></tr>`;
  } finally {
    if (btn) { btn.disabled = false; btn.innerHTML = 'Buscar'; }
  }
}

// â”€â”€â”€ Renderizar filas â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function renderTabla(rows, page) {
  const tbody = document.querySelector('#tableClientes tbody');
  tbody.innerHTML = '';

  if (!rows || rows.length === 0) {
    tbody.innerHTML = '<tr><td colspan="43" class="text-center text-muted py-3">Sin resultados</td></tr>';
    return;
  }
  ...
  rows.forEach(row => {
    ...
    tr.dataset.servicio = row.idServicio;
    tr.dataset.vehiculo = row.idVehiculo;
    tr.innerHTML = `
      ...
      <td>
        <button type="button" class="btn btn-success btn-sm" onclick="verCliente(this)">
...
function renderPaginacion(total, page, pageSize) {
  ...
  let html = `<div class="d-flex justify-content-between align-items-center flex-wrap gap-2 py-2">
    <small class="text-muted">Mostrando <strong>${desde}â€“${hasta}</strong> de <strong>${total}</strong> registros</small>`;
...
// â”€â”€â”€ Ver seguimiento del cliente â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Generar archivo Excel (re-fetch completo desde servidor) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function generarFile() {
  if (!Object.keys(_filters).length) {
    alert('Primero realice una bÃºsqueda antes de generar el archivo.');
    return;
  }
  const btn = document.querySelector('[onclick="generarFile()"]');
  if (btn) { btn.disabled = true; btn.textContent = 'Descargandoâ€¦'; }
  try {
    const resp = await fetch('../codigo/apiBuscarClientes.php', {
      method: 'POST',
      credentials: 'include',
...
function exportarExcel(rows) {
  const headers = [
    '#','Nombre','Fecha Inicio','Servicio','Asignado','Estado','Operador','IMEI','LÃ­nea',
    'DÃ­as Ren.','Fecha RenovaciÃ³n','Recarga','Fecha Recarga','InstalaciÃ³n','Instalador',
    'Valor InstalaciÃ³n','Pago InstalaciÃ³n','Valor Venta','MÃ©todo Pago','Realizar Factura',
    'Manejo','Ingreso Pago','RemisiÃ³n','Factura Nro.','ActualizaciÃ³n','Placa','Marca',
    'Referencia','Modelo','Cilindraje','Tipo IdentificaciÃ³n','Nro. IdentificaciÃ³n',
    'TelÃ©fono','Email','DirecciÃ³n','CÃ³mo se enterÃ³','Comercial',
