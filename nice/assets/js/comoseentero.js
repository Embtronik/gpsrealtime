const dataToSend = {
  ...
  fetch('../codigo/apitablecomoseentero.php', {
    method: 'POST',
    headers: {
    ...
    function eliminarFila(button) {
      ...
      const dataToSend = {
        idp_comoseentero: row.cells[0].innerText
      };
      
      fetch('../codigo/apiEliminarComoSeEntero.php', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    function agregarFila() {
      const tabla = document.getElementById('tabla-dinamica');
      const tbody = tabla.querySelector('tbody');
      const nameComercial = document.getElementById('name').value;
        ...
        filas.forEach((fila, indice) => {
          ...
          if (celda) { // Verificar si la celda es válida antes de acceder a su contenido
            ...
            if (valor === 'Pendiente') {
              ...
              fetch('../codigo/apiInsertarComoseentero.php', {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                },
