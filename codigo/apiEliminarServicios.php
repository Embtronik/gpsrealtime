...
if ($_SERVER['REQUEST_METHOD'] === 'POST') 
{
  ...
  $data = json_decode($jsonData, true);
    ...
    if (json_last_error() === JSON_ERROR_NONE) {
        $idp_servicios = $data['idp_servicios'];

        try {
                // Preparar el procedimiento almacenado
            $stmt = $conn->prepare("CALL sp_update_p_servicios(:idp_servicios)");

            // Enlazar los parámetros de entrada
            $stmt->bindParam(':idp_servicios', $idp_servicios);
            ...
            $response = array('success' => false, 'message' => 'Error al eliminar servicios: ' . $e->getMessage());
            header('Content-Type: application/json');
