...
if ($_SERVER['REQUEST_METHOD'] === 'POST') 
{
  ...
  $data = json_decode($jsonData, true);
    ...
    if (json_last_error() === JSON_ERROR_NONE) {
        $idp_comercial = $data['idp_comercial'];

        try {
                // Preparar el procedimiento almacenado
            $stmt = $conn->prepare("CALL sp_update_p_comercial(:idp_comercial)");

            // Enlazar los parámetros de entrada
            $stmt->bindParam(':idp_comercial', $idp_comercial);
            ...
            $response = array('success' => false, 'message' => 'Error insertar Comercial: ' . $e->getMessage());
            header('Content-Type: application/json');
